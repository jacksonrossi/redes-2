...
def verifica_temperatura():
    t = randrange(10) + 40
    #print('Verificando temperatura do Servidor (1) {} ... {}ºC'.format(NAME, t))
    logging.info('Verificando temperatura do Servidor (1) {} ... {}ºC'.format(NAME, t))
    time.sleep(2)
    return t
...
if __name__ == "__main__":
    ...
    logging.info('\n\n')
    logging.info('Servidor 1 - Iniciado no IP {} na porta {}'.format(HOST, PORT))

    while True:
        #aceita nova conexão
        conexao, cache = tcp.accept()
        #print('\nConexão realizada por:', cache)
        logging.info('Servidor 1 - Conexão realizada por: {}'.format(cache))
        ...
        logging.info('Servidor 1 - Finalizando conexão cache {}'.format(cache))
        conexao.close()
